...
@Component({
  selector: 'app-photo-editor-guest',
  standalone: true,
  templateUrl: './photo-editor-guest.html',
  imports: [CommonModule, FormsModule],
  styleUrls: ['./photo-editor-guest.scss']
})
export class PhotoEditorGuestComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (!id) return alert("Нет id фото!");

    // 📌 Загружаем только локальное фото
    if (id.startsWith("local_")) {
      const base64 = localStorage.getItem(id);
      if (!base64) return alert("Фото не найдено в локальном хранилище");

      this.photo = { id };
      this.loadImage(base64);
    } else {
      alert("Доступна только работа с локальными фото ❌");
    }
  }
  ...
  loadImage(url: string) {
    ...
    img.onerror = () => alert('Ошибка загрузки фото');
  }
  ...
  startCrop(event: MouseEvent) {
    ...
    this.cropStart = { x: event.clientX - rect.left, y: event.clientY - rect.top };
  ...
  onMouseMove(event: MouseEvent) {
    ...
    const rect = this.canvasRef.nativeElement.getBoundingClientRect();
    this.cropEnd = { x: event.clientX - rect.left, y: event.clientY - rect.top };
  ...
  async saveFinal() {
    ...
    this.bonusService.getBalance().subscribe({
      next: async (bonus) => {
        ...
        link.click();
        alert("Фото сохранено на ПК ✅");

        this.bonusService.spend(1).subscribe({
          next: (updated) => console.log("✅ Бонус списан. Остаток:", updated.value),
          error: (err) => console.error("❌ Ошибка при списании бонуса:", err)
        });
      },
      error: (err) => {
